    ...
    <DashboardLayout title="Faculty Directory" subtitle="Browse and rate your teachers">
      <div className={styles.page}>
        <div className={styles.toolbar}>
          ...
          <select className={styles.filterSelect} value={deptFilter}
        ...
        {loading ? (
          ...
          <EmptyState icon="👩‍🏫" title="No faculty found" description="Try adjusting your search or filter." />
        ) : (
          <div className={styles.teachersGrid}>
            {teachers.map((t, i) => {
              const badgeInfo = getBadgeInfo(undefined);
              return (
                <Link key={t.id} to={`/student/teachers/${t.id}`} className={styles.teacherCard}
                  style={{ animationDelay: `${i * 0.04}s` }}>
                  <div className={styles.cardTop}>
                    <Avatar name={t.name} src={t.avatar} size="lg" />
                    <div className={styles.scoreChip} style={{ color: getScoreColor(t.score), background: `${getScoreColor(t.score)}15` }}>
                      {t.score?.toFixed(0) || 0}%
                    </div>
                  </div>
