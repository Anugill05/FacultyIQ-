    ...
    <DashboardLayout>
      <div className={styles.page}>
        ...
        <div className={styles.statsGrid}>
          <StatCard icon="⭐" label="Average Rating"  value={`${data?.feedback_stats?.avg_rating ?? 0}/5`} iconBg="rgba(251,191,36,0.15)" delay={0} />
          <StatCard icon="💬" label="Total Reviews"   value={data?.feedback_stats?.total ?? 0}             iconBg="rgba(99,102,241,0.15)"  delay={0.1} />
          <StatCard icon="🏅" label="Achievements"    value={data?.achievements?.length ?? 0}               iconBg="rgba(34,197,94,0.15)"   delay={0.2} />
          <StatCard icon="🎯" label="Workshops"       value={data?.workshops_joined?.length ?? 0}           iconBg="rgba(167,139,250,0.15)" delay={0.3} />
        </div>
            ...
            <div className={styles.card}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <RadarChart data={radarData}>
                  ...
                  <PolarAngleAxis dataKey="subject" tick={{ fontSize: 12, fill: 'rgba(255,255,255,0.4)' }} />
                  <Radar
                    name="Score"
                    dataKey="A"
                    stroke="#6366f1"
              ...
              <div className={styles.scoreBarsList}>
                {[
                  { label: 'Student Rating', val: Math.round(ps?.student_rating_score    || 0), color: '#6366f1' },
                  { label: 'Attendance',     val: Math.round(ps?.attendance_score         || 0), color: '#22c55e' },
                  { label: 'Achievements',   val: Math.round(ps?.achievement_score        || 0), color: '#fbbf24' },
                  { label: 'Workshops',      val: Math.round(ps?.workshop_score           || 0), color: '#a78bfa' },
                ].map(s => (
              ...
              <div className={styles.feedbackList}>
                {data.feedback_stats.recent.map((f, i) => (
                  <div key={i} className={styles.feedbackItem}>
                    ...
                    {f.comment && (
                      <p className={styles.feedbackComment}>{truncate(f.comment, 80)}</p>
                    )}
              ...
              {data.announcements.map((a, i) => {
                ...
                return (
                  ...
                  >
                    <span>{st.icon}</span>
                    <div>
                      <strong style={{ fontSize: 14, color: '#e2e8f0' }}>{a.title}</strong>
